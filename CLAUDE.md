...
## Environment
Copy `.env.example` -> `.env`. Server vars: `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, `GEMINI_TTS_VOICE`, `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `TOKEN_ENCRYPTION_KEY` (32-byte hex / 64 chars), `SESSION_SECRET`, `ALLOWED_EMAILS`, `ALLOWED_ORIGIN`, `CEO_UID`, `ENGINE_CRON_SECRET`, `OBSIDIAN_VAULT_GIT_URL` (prod) or `OBSIDIAN_VAULT_PATH` (local), `FIREBASE_PROJECT_ID/CLIENT_EMAIL/PRIVATE_KEY`, `FINNHUB_API_KEY`. Frontend build-time (public): `VITE_FIREBASE_*`, feature flags `VITE_NEW_DASHBOARD` / `VITE_OPS_VIEWS`.
...
- One Firebase app. `src/lib/firestoreLemon.ts` used to point at a secondary legacy LEMON workspace (`lemon-es-tu-dios`) but now re-exports the primary `db` (`lemonDb = db`) and scopes Deals/Projects/Memory/Archive under `users/{uid}/...` like every other store. `isLemonWorkspaceConfigured()` is a back-compat stub that always returns true; the old `VITE_LEMON_FIREBASE_*` vars are gone.
