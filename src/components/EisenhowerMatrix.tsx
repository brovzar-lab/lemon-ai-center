...
const categoryColor: Record<string, string> = {
  DEAL: 'text-accent bg-accent/10',
  LEGAL: 'text-data-blue bg-data-blue/10',
  CREATIVE: 'text-data-coral bg-data-coral/10',
  OPS: 'text-data-teal bg-data-teal/10',
  FUND: 'text-ink-2 bg-sunken',
}
  ...
  return (
    <div className={`border ${borderClass} rounded-lg p-3`}>
      <div className="flex items-baseline gap-2 mb-2">
        <h4 className={`text-[11px] font-sans font-bold uppercase tracking-[0.15em] ${accentClass}`}>
          {label}
        </h4>
        <span className="text-[9px] font-sans text-ink-3">{sublabel}</span>
      </div>

      {items.length === 0 ? (
        <p className="text-[11px] font-sans text-ink-3 italic">—</p>
      ) : (
              ...
              >
                <div className="flex items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <p className="font-sans text-[12px] font-medium text-ink leading-snug truncate group-hover:text-accent transition-colors">
                      {item.title}
                    </p>
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="text-[10px] font-sans text-ink-3 truncate">
                        {item.from}
                      </span>
                      <span
                        className={`inline-block px-1 py-0 text-[8px] font-sans font-bold uppercase rounded ${categoryColor[item.category] || ''}`}
                      >
                        {item.category}
                      </span>
                    </div>
    ...
    <section aria-label="Eisenhower priority matrix" className="pb-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <Quadrant
          ...
          accentClass="text-data-coral"
          borderClass="border-data-coral/20"
        />
        <Quadrant
          label="Schedule"
          sublabel="important"
          items={data.important_not_urgent}
          accentClass="text-data-blue"
          borderClass="border-data-blue/20"
        />
        <Quadrant
          label="Delegate"
          sublabel="urgent"
